...
coalesce(nullif(trim(data_quality_flag),
''),
'MISSING') AS source_data_quality_flag,
_rescued_data,
_source_file_path,
_source_file_name,
_source_file_size,
...
CASE WHEN source_system NOT IN ('SAP_POS',
'LEGACY_POS_CSV') THEN 'ERR:INVALID_SOURCE_SYSTEM' END,
...
CASE WHEN order_status NOT IN ('Completed',
'Voided') THEN 'ERR:INVALID_ORDER_STATUS' END,
...
CASE WHEN payment_type NOT IN ('Card',
'Cash',
'Apple_Pay',
'Google_Pay',
'Voucher',
'Gift_Card') THEN 'ERR:INVALID_PAYMENT_TYPE' END,
CASE WHEN pos_terminal_id IS NULL OR pos_terminal_id = '' THEN 'ERR:MISSING_TERMINAL_ID' END,
CASE WHEN terminal_type NOT IN ('CASHIER',
'SELF_CHECKOUT') THEN 'ERR:INVALID_TERMINAL_TYPE' END,
...
try_cast(trim(original_unit_price_eur) AS DECIMAL(10,
2)) AS original_unit_price_eur,
...
CASE WHEN reason_code NOT IN ('Changed_Mind',
'Damaged',
'Wrong_Item',
'Defective',
'Expired') THEN 'ERR:INVALID_RETURN_REASON' END,
CASE WHEN ingestion_date IS NULL THEN 'ERR:INVALID_INGESTION_DATE' END
) AS hard_error_codes,
concat_ws('|',
...
AND abs(refund_amount_eur - round(net_unit_price_eur * return_quantity,
2)) > 0.02
...
CASE WHEN gender_code IS NOT NULL AND trim(gender_code) NOT IN ('M',
'F',
'D',
'U') THEN 'ERR:INVALID_GENDER_CODE' END,
CASE WHEN lower(trim(is_member)) NOT IN ('true',
'false',
'1',
'0') THEN 'ERR:INVALID_MEMBER_FLAG' END,
CASE WHEN lower(trim(is_member)) IN ('true',
'1') AND nullif(trim(loyalty_card_id),
'') IS NULL THEN 'ERR:MISSING_LOYALTY_CARD' END
) AS hard_error_codes,
concat_ws('|',
CASE WHEN try_cast(trim(age) AS INT) < 0 OR try_cast(trim(age) AS INT) > 120 THEN 'WARN:AGE_OUT_OF_RANGE' END,
CASE WHEN lower(trim(is_member)) IN ('false',
'0') AND nullif(trim(loyalty_card_id),
'') IS NOT NULL THEN 'WARN:CARD_FOR_NON_MEMBER' END
) AS warning_codes,
_rescued_data,
_source_file_path,
_source_file_name,
...
CASE WHEN lower(trim(is_private_label_eligible)) NOT IN ('true',
'false',
'1',
'0') THEN 'ERR:INVALID_PRIVATE_LABEL_ELIGIBILITY' END,
CASE WHEN try_cast(trim(price_min_eur) AS DECIMAL(10,
2)) IS NULL THEN 'ERR:INVALID_MIN_PRICE' END,
CASE WHEN try_cast(trim(price_max_eur) AS DECIMAL(10,
2)) IS NULL THEN 'ERR:INVALID_MAX_PRICE' END,
CASE WHEN try_cast(trim(price_min_eur) AS DECIMAL(10,
2)) < 0 THEN 'ERR:NEGATIVE_MIN_PRICE' END,
CASE WHEN try_cast(trim(price_max_eur) AS DECIMAL(10,
2)) < try_cast(trim(price_min_eur) AS DECIMAL(10,
2)) THEN 'ERR:PRICE_RANGE_REVERSED' END,
CASE WHEN unit IS NULL OR trim(unit) = '' THEN 'ERR:MISSING_UNIT' END,
CASE WHEN try_cast(trim(vat_rate) AS DECIMAL(5,
4)) NOT IN (0.0700,
0.1900) THEN 'ERR:INVALID_VAT_RATE' END
) AS hard_error_codes,
_rescued_data,
_source_file_path,
_source_file_name,
...
CASE WHEN subcategory IS NULL OR trim(subcategory) = '' THEN 'ERR:MISSING_SUBCATEGORY' END,
CASE WHEN default_brand IS NULL OR trim(default_brand) = '' THEN 'ERR:MISSING_DEFAULT_BRAND' END,
CASE WHEN try_cast(trim(effective_price_eur) AS DECIMAL(10,
2)) IS NULL OR try_cast(trim(effective_price_eur) AS DECIMAL(10,
2)) <= 0 THEN 'ERR:INVALID_EFFECTIVE_PRICE' END,
...
CASE WHEN lower(trim(is_promo_price)) NOT IN ('true',
'false',
'1',
'0') THEN 'ERR:INVALID_PROMO_PRICE_FLAG' END,
CASE WHEN unit IS NULL OR trim(unit) = '' THEN 'ERR:MISSING_UNIT' END,
CASE WHEN try_cast(trim(vat_rate) AS DECIMAL(5,
4)) NOT IN (0.0700,
0.1900) THEN 'ERR:INVALID_VAT_RATE' END
...
CAST(0 AS BIGINT),
round(100.0 * (SELECT count(*) FROM dim_stores_quarantine) / nullif((SELECT count(*) FROM dim_stores_raw),
0),
4),
current_timestamp()
UNION ALL
...
round(100.0 * (SELECT count(*) FROM dim_customers_quarantine) / nullif((SELECT count(*) FROM dim_customers_raw),
0),
4),
current_timestamp()
UNION ALL
...
CAST(0 AS BIGINT),
round(100.0 * (SELECT count(*) FROM dim_products_quarantine) / nullif((SELECT count(*) FROM dim_products_raw),
0),
4),
current_timestamp()
UNION ALL
...
CAST(0 AS BIGINT),
round(100.0 * (SELECT count(*) FROM dim_products_scd2_quarantine) / nullif((SELECT count(*) FROM dim_products_scd2_raw),
0),
4),
current_timestamp();
