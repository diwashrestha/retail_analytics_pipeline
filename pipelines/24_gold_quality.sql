...
SELECT
  ...
  cast(count(*) - count(DISTINCT concat_ws('||', cast(order_date AS STRING), store_id, category, subcategory)) AS DECIMAL(20,4)) AS actual_value,
  CASE WHEN count(*) = count(DISTINCT concat_ws('||', cast(order_date AS STRING), store_id, category, subcategory)) THEN 'PASSED' ELSE 'FAILED' END AS status,
...
SELECT
  ...
  'Product performance must contain one row per trusted product.', current_timestamp()
...
SELECT
  ...
  cast(count(*) - count(DISTINCT concat_ws('||', product_id, reason_code)) AS DECIMAL(20,4)),
  CASE WHEN count(*) = count(DISTINCT concat_ws('||', product_id, reason_code)) THEN 'PASSED' ELSE 'FAILED' END,
  'Return analysis must contain one row per product-reason combination.', current_timestamp()
FROM return_analysis

UNION ALL
SELECT
  'hourly_traffic_unique_grain', 'CRITICAL', cast(0 AS DECIMAL(20,4)),
  cast(count(*) - count(DISTINCT concat_ws('||', cast(order_hour AS STRING), cast(weekday_number AS STRING), store_size_class)) AS DECIMAL(20,4)),
  CASE WHEN count(*) = count(DISTINCT concat_ws('||', cast(order_hour AS STRING), cast(weekday_number AS STRING), store_size_class)) THEN 'PASSED' ELSE 'FAILED' END,
  'Hourly traffic must contain one row per hour-weekday-store-size combination.', current_timestamp()
FROM hourly_traffic
...
UNION ALL
SELECT
  'gold_non_negative_values', 'CRITICAL', cast(0 AS DECIMAL(20,4)), cast(count(*) AS DECIMAL(20,4)),
  CASE WHEN count(*) = 0 THEN 'PASSED' ELSE 'FAILED' END,
  'Gold financial and quantity metrics must not become negative.', current_timestamp()
FROM (
...
SELECT
  ...
  CASE WHEN count(ltv_rank_top_1000) = least(1000, count(*)) THEN 'PASSED' ELSE 'FAILED' END,
  'Top-1000 LTV rank must contain exactly min(1000, trusted customer count) populated ranks.', current_timestamp()
FROM customer_ltv

UNION ALL
SELECT
  'customer_rank_unique', 'CRITICAL', cast(0 AS DECIMAL(20,4)),
  cast(count(overall_ltv_rank) - count(DISTINCT overall_ltv_rank) AS DECIMAL(20,4)),
...
UNION ALL
SELECT
  'return_reason_shares', 'WARNING', cast(0 AS DECIMAL(20,4)), cast(count(*) AS DECIMAL(20,4)),
  CASE WHEN count(*) = 0 THEN 'PASSED' ELSE 'FAILED' END,
  'For every product with returns, return-event shares should sum to approximately 100 percent.', current_timestamp()
FROM (
  SELECT product_id
  FROM return_analysis
  GROUP BY product_id
...
SELECT
  ...
  cast(CASE WHEN coalesce(sum(net_sales_eur), 0) > 0 THEN 1 ELSE 0 END AS DECIMAL(20,4)),
  CASE WHEN coalesce(sum(net_sales_eur), 0) > 0 THEN 'PASSED' ELSE 'FAILED' END,
  'A demo or portfolio run should produce positive Gold revenue.', current_timestamp()
FROM daily_sales;
