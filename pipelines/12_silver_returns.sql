...
SELECT
  ...
  round(sum(net_sales_eur) / nullif(sum(quantity), 0), 4) AS original_net_unit_price_eur,
...
SELECT
  ...
  concat_ws('|',
    ...
    CASE
      WHEN sale_transaction_id IS NOT NULL
       AND datediff(return_date, sale_order_date) > cast(:max_return_window_days AS INT)
...
AS
SELECT 'bronze_return_rows' AS metric_name, cast(bronze_rows AS DECIMAL(20,2)) AS metric_value, measured_at
FROM silver_return_reconciliation
UNION ALL
SELECT 'return_id_conflict_rows', cast(id_conflict_rows AS DECIMAL(20,2)), measured_at
FROM silver_return_reconciliation
UNION ALL
SELECT 'exact_duplicate_returns_removed', cast(exact_duplicate_rows AS DECIMAL(20,2)), measured_at
FROM silver_return_reconciliation
UNION ALL
SELECT 'trusted_return_rows', cast(trusted_return_rows AS DECIMAL(20,2)), measured_at
FROM silver_return_reconciliation
UNION ALL
SELECT 'return_review_rows', cast(return_review_rows AS DECIMAL(20,2)), measured_at
FROM silver_return_reconciliation
UNION ALL
SELECT 'return_reconciliation_difference', cast(reconciliation_difference AS DECIMAL(20,2)), measured_at
FROM silver_return_reconciliation
UNION ALL
SELECT 'trusted_refund_amount_eur', cast(round(sum(refund_amount_eur), 2) AS DECIMAL(20,2)), current_timestamp()
FROM fact_returns;
