...
round(unit_price_eur * quantity * (discount_pct / 100),
2) AS calculated_discount_eur,
round(unit_price_eur * quantity * (1 - discount_pct / 100),
2) AS calculated_net_sales_eur,
round(unit_price_eur * quantity - net_revenue_eur,
2) AS source_discount_amount_eur,
...
round(net_revenue_eur - (net_revenue_eur / (1 + effective_vat_rate)),
2) AS vat_amount_eur,
concat_ws('|',
...
CASE
WHEN abs(net_revenue_eur - round(unit_price_eur * quantity * (1 - discount_pct / 100),
2))
...
AND abs(unit_price_eur - effective_price_eur) > cast(:price_tolerance_eur AS DECIMAL(10,
2))
...
AS
SELECT 'bronze_transaction_rows' AS metric_name,
cast(bronze_rows AS DECIMAL(20,
2)) AS metric_value,
measured_at
FROM silver_transaction_reconciliation
UNION ALL
SELECT 'record_hash_conflict_rows',
cast(hash_conflict_rows AS DECIMAL(20,
2)),
measured_at
FROM silver_transaction_reconciliation
UNION ALL
SELECT 'exact_duplicate_rows_removed',
cast(exact_duplicate_rows AS DECIMAL(20,
2)),
measured_at
FROM silver_transaction_reconciliation
UNION ALL
SELECT 'trusted_sales_rows',
cast(valid_sales_rows AS DECIMAL(20,
2)),
measured_at
FROM silver_transaction_reconciliation
UNION ALL
SELECT 'sales_review_rows',
cast(sales_review_rows AS DECIMAL(20,
2)),
measured_at
FROM silver_transaction_reconciliation
UNION ALL
...
FROM silver_transaction_reconciliation
UNION ALL
SELECT 'transaction_reconciliation_difference',
cast(reconciliation_difference AS DECIMAL(20,
2)),
measured_at
FROM silver_transaction_reconciliation
UNION ALL
SELECT 'trusted_net_sales_eur',
cast(round(sum(net_sales_eur),
2) AS DECIMAL(20,
2)),
current_timestamp()
FROM fact_sales;
