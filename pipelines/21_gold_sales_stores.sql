...
round(try_divide(sum(net_sales_eur),
count(DISTINCT basket_id)),
2) AS category_revenue_per_basket_eur,
round(try_divide(sum(net_sales_eur),
sum(quantity)),
2) AS weighted_average_selling_price_eur,
round(try_divide(sum(discount_amount_eur),
sum(pre_discount_sales_eur)) * 100,
2) AS weighted_discount_rate_pct,
round(sum(CASE WHEN is_private_label THEN net_sales_eur ELSE 0 END),
2) AS private_label_sales_eur,
round(try_divide(
...
round(sum(CASE WHEN customer_type = 'Walk-in' THEN net_sales_eur ELSE 0 END),
2) AS walk_in_sales_eur,
round(sum(CASE WHEN customer_type = 'Loyalty Member' THEN net_sales_eur ELSE 0 END),
2) AS member_sales_eur,
round(sum(CASE WHEN customer_type = 'Registered Non-member' THEN net_sales_eur ELSE 0 END),
2) AS registered_nonmember_sales_eur,
...
round(try_divide(discount_amount_eur,
pre_discount_sales_eur) * 100,
2) AS discount_rate_pct,
round(try_divide(walk_in_baskets,
basket_count) * 100,
2) AS walk_in_basket_share_pct,
round(try_divide(member_baskets,
basket_count) * 100,
2) AS member_basket_share_pct,
round(try_divide(self_checkout_baskets,
basket_count) * 100,
2) AS self_checkout_basket_share_pct,
round(try_divide(promo_period_baskets,
basket_count) * 100,
2) AS promo_basket_share_pct,
round(try_divide(private_label_sales_eur,
net_sales_eur) * 100,
...
units_sold) * 100,
2) AS returned_unit_rate_pct,
round(try_divide(net_sales_eur,
active_sales_days),
2) AS revenue_per_active_day_eur,
