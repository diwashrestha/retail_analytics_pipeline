...
coalesce(b.lifetime_pre_discount_sales_eur,
0) AS lifetime_pre_discount_sales_eur,
...
coalesce(b.lifetime_private_label_sales_eur,
0) AS lifetime_private_label_sales_eur,
...
round(try_divide(lifetime_discount_amount_eur,
lifetime_pre_discount_sales_eur) * 100,
2) AS lifetime_discount_rate_pct,
round(try_divide(lifetime_private_label_sales_eur,
lifetime_sales_eur) * 100,
2) AS private_label_sales_share_pct,
round(try_divide(promo_baskets,
lifetime_baskets) * 100,
2) AS promo_basket_share_pct,
round(try_divide(coupon_baskets,
lifetime_baskets) * 100,
2) AS coupon_basket_share_pct,
round(try_divide(self_checkout_baskets,
lifetime_baskets) * 100,
2) AS self_checkout_basket_share_pct,
round(try_divide(refund_amount_eur,
lifetime_sales_eur) * 100,
...
row_number() OVER (
ORDER BY (lifetime_sales_eur - refund_amount_eur) DESC,
lifetime_sales_eur DESC,
customer_id
) AS overall_ltv_rank,
percent_rank() OVER (
ORDER BY (lifetime_sales_eur - refund_amount_eur) DESC,
...
SELECT
*,
round(try_divide(return_event_count,
sum(return_event_count) OVER (PARTITION BY product_id)) * 100,
2) AS product_return_event_share_pct,
round(try_divide(returned_units,
sum(returned_units) OVER (PARTITION BY product_id)) * 100,
2) AS product_returned_unit_share_pct,
round(try_divide(refund_amount_eur,
sum(refund_amount_eur) OVER (PARTITION BY product_id)) * 100,
2) AS product_refund_share_pct,
round(try_divide(return_event_count,
sum(return_event_count) OVER ()) * 100,
4) AS all_return_event_share_pct,
round(try_divide(refund_amount_eur,
sum(refund_amount_eur) OVER ()) * 100,
4) AS all_refund_share_pct,
dense_rank() OVER (PARTITION BY product_id ORDER BY return_event_count DESC,
reason_code) AS reason_rank_within_product,
current_timestamp() AS gold_refreshed_at
FROM reason_metrics;
...
count_if(coupon_applied) AS coupon_baskets,
round(try_divide(count_if(customer_type = 'Walk-in'),
count(*)) * 100,
2) AS walk_in_basket_share_pct,
round(try_divide(count_if(customer_type = 'Loyalty Member'),
count(*)) * 100,
2) AS member_basket_share_pct,
round(try_divide(count_if(is_self_checkout),
count(*)) * 100,
2) AS self_checkout_basket_share_pct,
round(try_divide(count_if(is_promo_period),
count(*)) * 100,
2) AS promo_basket_share_pct,
current_timestamp() AS gold_refreshed_at
FROM basket_analysis
