...
from price_history import PriceIndex, write_scd2
from price_history import validate as validate_scd2
...
def compute_daily_volumes(
    start: datetime, end: datetime, n_total: int, rng: Random
) -> dict[datetime, int]:
...
def schedule_late_arrival(
    basket: list[dict], current_date: datetime, end: datetime, rng: Random
) -> datetime | None:
...
def write_incremental(args, out_dir: Path) -> dict:
    ...
    print(
        f"  [1/4] Loading masters + building {args.customers:,} customers ...",
        flush=True,
    )
    ...
    print("  [4/4] Writing daily batch files ...", flush=True)
    ...
    bar = ProgressBar(total=len(sorted_days), unit="days", label="Generating batches")
    ...
    for day_idx, current_date in enumerate(sorted_days):
        ...
        while emitted_today < target_rows:
            is_duplicate = (
                bool(recent_day_baskets) and rng.random() < args.duplicate_rate
            )
            if is_duplicate:
        ...
        bar.update(
            1,
            extra=f"{current_date.strftime('%Y-%m-%d')}  "
    ...
    if overflow_rows:
        ...
        print(
            f"  overflow batch    : {len(overflow_rows):,} rows (late, past end_date)"
        )
...
def scan_batches(
    out_dir: Path, stats: dict, target_late: float
...
def validate(
    out_dir: Path, stats: dict, late_rate: float, start: datetime, end: datetime
) -> bool:
